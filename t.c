...
#define TESTBASE 500
	{
		...
		for (i = 0; i < TESTBASE; i++) {
			...
			for (j = 0; j < nb; j++) {
				fprintf(tst, " %g", fT  (a, b, j, j, a + i * dx));
	...
	for (j = 0; j < nb; j++) {
		for (i = 0; i < nb; i++)
			for (k = 0; k < pts->n; k++)
				add_to_entry_matrix(eqs, j, i, fT(a, b, i, i, x[k]) * fT(a, b, j, j, x[k]));

		for (k = 0; k < pts->n; k++)
			add_to_entry_matrix(eqs, j, nb, y[k] * fT(a, b, j, j, x[k]));
	}
...
#ifdef DEBUG
	{
		...
		for (i = 0; i < TESTBASE; i++) {
			...
			double xi= a + i * dx;
			for( k= 0; k < nb; k++ ) {
							yi += get_entry_matrix(eqs, k, nb) * fT(a, b, k, k, xi);
